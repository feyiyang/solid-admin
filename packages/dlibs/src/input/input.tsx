...
const Root: Component<InputRootProp> = (props) => {
  ...
  const handleClear = () => {
    props.onChange ? props.onChange('') : setInitValue('')
  }
  const clearIsHidden = () => !props.value && !initValue()
  const clazz = local.multiline
    ? 'enn-textarea enn-textarea-bordered'
    : `enn-input enn-input-bordered gap-2 ${sizeVariants[local.size]}`
  return (
...
const Input: Component<ComponentProps<any>> = (props) => {
  ...
  return (
    <>
      <Dynamic
        component={context.multiline ? TextField.TextArea : TextField.Input}
        class={`grow outline-none ${local.class || ''}`}
        {...props}
      />
      {/* <TextField.Input /> */}
      <Show when={context.clearable && !context.hidden()}>
        <span
          class="icon-[tdesign--close-circle-filled] btn-clear"
          classList={{ hidden: context.hidden() }}
