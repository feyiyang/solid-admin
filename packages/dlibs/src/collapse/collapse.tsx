...
const Root: Component<collapseProps & ComponentProps<'div'>> = (props) => {
  ...
  return (
    <Accordion.Root
      class={`enn-collapse-root enn-join enn-join-vertical ${local.class || ''}`}
      collapsible
      multiple={!local.mutex}
      {...rest}
    >
...
const Title: Component<ComponentProps<any>> = (props) => {
  ...
  return (
    <Accordion.Trigger
      class={`enn-collapse-title ${local.class || ''}`}
      as={Accordion.Header}
      {...rest}
    />
  )
...
const Content = (props: Accordion.AccordionContentProps) => (
  <Accordion.Content class="enn-collapse-content" {...props} />
)
